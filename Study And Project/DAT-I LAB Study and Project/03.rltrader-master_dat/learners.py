...
class ReinforcementLearner:
    ...
    def __init__(self, rl_method='rl', stock_code=None, 
        ...
        logging.info("self.chart_data [{}]".format(self.chart_data))
        ...
        logging.info("self.training_data [{}]".format(self.training_data))
        
        self.sample = None
        self.training_data_idx = -1
        # 벡터 크기 = 학습 데이터 벡터 크기 + 에이전트 상태 크기
        self.num_features = self.agent.STATE_DIM    # 주식 보유 비율, 포트폴리오 가치 비율
    ...
    def build_sample(self):
        ...
        if len(self.training_data) > self.training_data_idx + 1:
            ...
            logging.info("build_sample [{}][{}][{}]"
                .format(len(self.training_data), self.training_data_idx, self.training_data.iloc[self.training_data_idx].tolist()))
    ...
    def run(self, num_epoches=100, balance=10000000,
        ...
        for epoch in range(num_epoches):
            ...
            while True:
                ...
                # 매수매도 및 수량
                # trading_unit local variable 
                immediate_reward, delayed_reward, action, rtn_trading_unit = \
                    self.agent.act(action, confidence)

                logging.info("action[{}] rtn_trading_unit[{}] immediate_reward[{}] delayed_reward[{}] confidence[{}] exploration[{}] spred_value[{}] pred_policy[{}] [{}]"
                    .format(action, rtn_trading_unit, immediate_reward, delayed_reward, confidence, exploration, pred_value, pred_policy, next_sample))
    ...
    def save_models(self):
        ...
        if self.value_network is not None and self.value_network_path is not None:
            ...
            logging.info("self.value_network[{}][{}]".format(self.value_network, self.value_network_path))
            self.value_network.save_model(self.value_network_path)
        if self.policy_network is not None and self.policy_network_path is not None:
            
            logging.info("self.policy_network[{}][{}]".format(self.policy_network, self.policy_network_path))
...
class A3CLearner(ReinforcementLearner):
    ...
    def run(
        self, num_epoches=3, balance=10000000,
        discount_factor=0.9, start_epsilon=0.5, learning=True):
        threads = []
