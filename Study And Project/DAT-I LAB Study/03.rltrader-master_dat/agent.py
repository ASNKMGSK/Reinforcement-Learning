...
class Agent:
    ...
    # 행동
    # 2020.11.26
    # 체결 테이블 'L_OR_S' 칼럼에서 값들이 0, 1, 2 로 되어 있는데 L/S 은 1, 2 인가요? 
    # 살짝 헷갈리게 되어 잇는데. 1이 매도, 2가 매수
    # API 기준 
    ACTION_BUY = 1  # 매수
    ACTION_SELL = 2  # 매도
    ACTION_HOLD = 0  # 홀딩
    ...
    def act(self, action, confidence):
        # 주식매수인 경우 보유금액 있는지 확인
        # 주식매도인 경우 보유주식수 있는 확인
        ...
        if action == Agent.ACTION_BUY:
            ...
            logging.info("Agent.ACTION_BUY action[{}] confidence[{}] trading_unit[{}] balance[{}] invest_amount[{}]"
                    .format(action, confidence, trading_unit, balance, invest_amount))
        # 매도
        elif action == Agent.ACTION_SELL:
            # 매도할 단위를 판단
            trading_unit = self.decide_trading_unit(confidence)
            ...
            logging.info("Agent.ACTION_SELL action[{}] confidence[{}] trading_unit[{}] balance[{}] invest_amount[{}]"
                    .format(action, confidence, trading_unit, balance, invest_amount))
        # 홀딩
        # 보유 주식수 추가
            ...
            logging.info("Agent.ACTION_HOLD action[{}] self.num_hold[{}]"
                        .format(action, self.num_hold))
        ...
        return self.immediate_reward, delayed_reward, action, trading_unit
